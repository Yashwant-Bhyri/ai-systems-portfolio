...
export function BenchMini({ idx, t }: { idx: number; t: number }) {
  if (idx === 0) {
    ...
    return (
      <g>
        <text x={10} y={18} className="svg-sub tiny">{T("EMBEDDED AUDIO INTELLIGENCE · LIVE")}</text>
        ...
        <text x={78} y={78} className="svg-sub tiny">{T("log-Mel features")}</text>
        ...
        <text x={158} y={78} className="svg-sub tiny">{T("pruned model")}</text>
        ...
        <g className={`lv-chip ${Math.floor(t / 1600) % 2 ? "win" : ""}`}>
          ...
          <text x={264} y={45} textAnchor="middle" className="svg-mono tinytext">{T("TFLM INT8")}</text>
          <text x={264} y={57} textAnchor="middle" className="svg-sub tiny">{T("edge SoC")}</text>
        </g>
        {[[T("95%+ accuracy"), 10], [T("sub-10 ms"), 112], [T("4× compression"), 208]].map(([v, x]) => (
          <text key={String(v)} x={Number(x)} y={112} className="svg-mono tinytext">{v}</text>
        ))}
        <text x={10} y={136} className="svg-sub tiny">{T("inference")} {fmtTick(t)} · {Math.floor(t / 800) % 2 ? T("keyword detected ✓") : T("listening…")}</text>
      </g>
    );
  }
  if (idx === 1) {
    ...
    return (
      <g>
        <text x={8} y={15} className="svg-sub tiny">{T("SAM + INT8 DEPTH → 5-TEXTURE WEBGL CONTRACT")}</text>
        ...
        <text x={37} y={66} textAnchor="middle" className="svg-sub tiny">{T("CAMERA")}</text>
        ...
        <text x={111} y={66} textAnchor="middle" className="svg-sub tiny">{T("SAM MASK")}</text>
        ...
        <text x={185} y={66} textAnchor="middle" className="svg-sub tiny">{T("DEPTH V2 · INT8")}</text>
        <text x={219} y={51} className="svg-sub tiny">→</text>
        <g className="lv-chip win">
          <rect x={230} y={24} width={82} height={48} rx={8} />
          <text x={271} y={44} textAnchor="middle" className="svg-mono tinytext">{T("GPU CONTRACT")}</text>
          <text x={271} y={59} textAnchor="middle" className="svg-sub tiny">{T("5 TEXTURES")}</text>
        </g>
        <text x={8} y={92} className="svg-mono tinytext">{T("LIVE LANE")}</text>
        ...
        <text x={312} y={92} textAnchor="end" className="svg-sub tiny">{T("SUBJECT LIFT")}</text>
        <text x={8} y={115} className="svg-mono tinytext">{T("HQ LANE")}</text>
        ...
        <text x={312} y={115} textAnchor="end" className="svg-sub tiny">{T("SCENE CACHE")}</text>
        <text x={160} y={140} textAnchor="middle" className="svg-sub tiny">{T("CUT GUARD ✦ CACHE BLEND ✦ MEMORY BUDGET")}</text>
      </g>
    );
  }
  if (idx === 2) {
    ...
    return (
      <g>
        <text x={10} y={18} className="svg-sub tiny">{T("CONTROLLED VIDEO R&D ·")} {regen ? T("REGEN BRANCH") : T("PROMPT STEERING")}</text>
        {[0, 1, 2, 3].map((f2) => (
          <g key={f2} className={`lv-node ${frame === f2 ? "is-live" : ""}`}>
            ...
            <text x={40 + f2 * 70} y={64} textAnchor="middle" className="svg-sub tiny">{[T("hook"), T("reveal"), T("detail"), T("cta")][f2]}</text>
          </g>
        ))}
        ...
        <g className={`lv-chip ${regen ? "win" : ""}`}>
          ...
          <text x={80} y={112} textAnchor="middle" className="svg-mono tinytext">{T("SOFT · reuse seed 4271")}</text>
        </g>
        <g className={`lv-chip ${regen ? "" : "win"}`}>
          <rect x={160} y={96} width={148} height={25} rx={8} />
          <text x={234} y={112} textAnchor="middle" className="svg-mono tinytext">{T("HARD · assign new seed")}</text>
        </g>
        <text x={10} y={136} className="svg-sub tiny">{T("typed control layer · provider output remains best-effort")}</text>
        <text x={10} y={148} className="svg-mono tinytext">{T("PROJECT + SHOT STYLE + GLOBAL CONSISTENCY")}</text>
      </g>
    );
  }
  ...
  return (
    <g>
      <text x={10} y={18} className="svg-sub tiny">{T("EXECUTION-GROUNDED SQL · + DISTILL GATE")}</text>
      ...
      <text x={20} y={100} className={exOk ? "tick ok" : "svg-sub tiny"}>{exOk ? T("EX ✓ executed on live DB") : T("compiling…")}</text>
      ...
      <text x={200} y={70} className="svg-mono tinytext">{T("judge:")} {judge === 1 ? T("✗ reject") : T("✓ accept")}</text>
      <g className="lv-chip win">
        <rect x={200} y={80} width={70} height={22} rx={7} />
        <text x={235} y={95} textAnchor="middle" className="svg-mono tinytext">{T("→ SLM")}</text>
      </g>
      <text x={10} y={126} className="svg-mono tinytext">{T("12,751+ pairs · 95+ DBs")}</text>
      <text x={10} y={142} className="svg-mono tinytext">{T("31% fewer factual errors")}</text>
    </g>
  );
}
...
export function LogisticsMiniGraphic({ t: elapsed }: { t: number }) {
  ...
  const pulses = TD(ROLE_REQS).map((_, index) => {
  ...
  const prompt = refusalPhase
    ? typed(T("> export customer records"), t, 5600, 7000)
    : typed(T("> late deliveries · Kowloon"), t, 800, 2400);
  ...
  const status = refused
    ...
    : refusalPhase
      ...
      : t >= 2700
        ...
        : T("WAITING FOR APPROVED INTENT");
  const statusClass = refused ? "tick bad" : t >= 2700 && !refusalPhase ? "tick ok" : "svg-sub tiny";

  return (
    <g>
      <text x={8} y={15} className="svg-sub tiny">{T("LALAMOVE R&amp;D · BOUNDED AI OPERATIONS")}</text>
      <text x={312} y={15} textAnchor="end" className="svg-sub tiny">{TD(MINI_WEATHER)[weatherIndex]}</text>

      {TD(ROLE_REQS).map((request, index) => {
        const y = 25 + index * 25;
        ...
        return (
          <g key={request.role}>
            ...
            <text x={36} y={y + 15} textAnchor="middle" className="svg-mono tinytext">{request.role}</text>
      ...
      <text x={115} y={60} textAnchor="middle" className="svg-label small">{T("FLASK API")}</text>
      <text x={115} y={75} textAnchor="middle" className="svg-sub tiny">
        {lastLanded >= 0 ? MINI_ENDPOINTS[lastLanded] : T("ROLE GATE")}
      </text>
      ...
      <g className={`lv-chip ${rerouted ? "win" : ""}`}>
        ...
        <text x={192} y={39} textAnchor="middle" className="svg-sub tiny">{T("ROUTE RISK")}</text>
      ...
      <text x={8} y={145} className="svg-sub tiny">{T("BOUND SQL ✦ VISIBLE REFUSALS ✦ HUMAN ACTION")}</text>
    </g>
  );
}
...
export function LogisticsGraphic({ a }: { a: boolean }) {
  ...
  const pulses = TD(ROLE_REQS).map((_, i) => {
  ...
  const q1 = typed(T("> show late deliveries in Kowloon"), t, 800, 2400);
  const q2 = typed(T("> export all customer records"), t, 5600, 7000);

  return (
    <svg viewBox="0 0 640 320" className="op-svg">
      {TD(ROLE_REQS).map((r, i) => {
        const y = 46 + i * 64;
        const pp = pulses[i].p;
        ...
        return (
          <g key={r.role}>
            ...
            <text x={82} y={y + 29} textAnchor="middle" className="svg-sub tiny">{T("JWT role")}</text>
      ...
      <text x={312} y={144} textAnchor="middle" className="svg-label small">{T("FLASK API")}</text>
      <text x={312} y={162} textAnchor="middle" className="svg-mono tinytext">
        {lastLanded >= 0 ? TD(ROLE_REQS)[lastLanded].req : "role-gated"}
      </text>
      ...
      <g transform="translate(300,246)">
        ...
        <text x={0} y={2} textAnchor="middle" className="svg-sub tiny">{T("route risk")}</text>
        <text x={0} y={22} textAnchor="middle" className="svg-mono tinytext">{TD(WEATHER)[wIdx]}</text>
      </g>
      {storm && risk > 0.75 && (
        <g className="lv-chip win" style={rise(eph(t % 2750, 1500, 1900), 6)}>
          <rect x={382} y={236} width={104} height={26} rx={8} />
          <text x={434} y={253} textAnchor="middle" className="svg-mono tinytext">{T("re-routed ✓")}</text>
        </g>
      )}
      ...
      <g className="lv-box">
        ...
        <text x={414} y={52} className="svg-sub">{T("AI COPILOT · whitelisted intents only")}</text>
        <text x={414} y={76} className="svg-mono tinytext">{q1}{t > 800 && t < 2500 ? caret(t) : ""}</text>
        {t > 2800 && <text x={414} y={96} className="tick ok" style={rise(eph(t, 2800, 3100), 4)}>{T("intent: delivery_status ✓ → parameterized query")}</text>}
        {t > 3400 && <text x={414} y={116} className="svg-mono tinytext" style={rise(eph(t, 3400, 3700), 4)}>{T("KLN-204 · 41 min late · storm")}</text>}
        {t > 3900 && <text x={414} y={134} className="svg-mono tinytext" style={rise(eph(t, 3900, 4200), 4)}>{T("KLN-217 · 12 min late")}</text>}
        {t > 4400 && <text x={414} y={150} className="svg-sub tiny" style={rise(eph(t, 4400, 4700), 4)}>{T("4 rows returned · 0 unrestricted queries")}</text>}
        <text x={414} y={168} className="svg-mono tinytext">{q2}{t > 5600 && t < 7100 ? caret(t) : ""}</text>
        {t > 7400 && <text x={414} y={186} className="tick bad" style={rise(eph(t, 7400, 7700), 4)}>{T("✗ intent not whitelisted — refused")}</text>}
        {t > 8200 && <text x={414} y={204} className="svg-sub tiny" style={rise(eph(t, 8200, 8500), 4)}>{T("bounded AI · approved handlers only")}</text>}
      </g>

      <text x={24} y={306} className="svg-note">
        {T("Role-gated APIs · weather-aware route risk · bounded copilot with visible refusals")}
      </text>
    </svg>
  );
}
  ...
  return (
    <section ref={sectionRef} data-chapter="systems" id="systems" className="fable-section systems-chapter" style={{ "--accent": "#ffd166" } as React.CSSProperties}>
      <header className="chapter-head">
        <p className="chapter-kicker">{T("PROJECT 04 + THE BENCH")}</p>
        <h2 className="chapter-title">{T("Heavy machinery &amp; research bench")}</h2>
        <p className="chapter-sub">
          {T("The same discipline, applied to operations software and to research prototypes.")}
        </p>
      </header>
      <div className="systems-grid">
        <div className="systems-main">
          <h3 className="systems-name">{T("Logistics AI-Ops Platform")}</h3>
          <p className="systems-plain">
            {T("Role-gated logistics operations — admin, driver, and customer workflows over MySQL, with weather- and\n            geo-aware route risk scoring and an AI copilot that is deliberately allowed to do very few things.")}
          </p>
          <LogisticsGraphic a={onScreen} />
        </div>
        <div className="systems-bench">
          <h3 className="systems-name">{T("Research bench")}</h3>
          ...
          <div className="bench-mini-stage" key={lit}>
            <svg viewBox="0 0 320 150" className="op-svg bench-mini" aria-label={T("Live micro-visual of the selected research project")}>
